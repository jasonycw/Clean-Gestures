...
var generateTab = function (state, length) {
  ...
  if (state.root) tab.css({
    "background-color": "#aaeeaa"
  });
  else if (state.tabStatus == "loading") tab.text("...").css({
    "background-color": "#eeeeaa"
  });
  else if (!state.goodurl) tab.css({
...
var reloadall = function () {
  ...
  if (states[id][i].title && !states[id][i].root && states[id][i].goodurl) reloadTab(states[id][i].tabId);
