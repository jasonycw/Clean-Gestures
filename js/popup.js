    ...
    bg.getTabStatus(tab.id, function (stat) {
      if (stat == "broken") {
        $("body").append("<div id=statuslight class=red>").append($("<h1>").text(chrome.i18n.getMessage('popup_status_broken_short')))
          ...
          .append($('<div class=sgbutton>').text(chrome.i18n.getMessage('popup_button_options')).css({
          "float": "right"
        }).click(function () {
          chrome.tabs.create({
            "url": "options.html"
          ...
          .append($('<div class=sgbutton>').text(chrome.i18n.getMessage('popup_button_reload')).css({
          "float": "left"
        }).click(reloadPage))
      } else if (stat == "unable") {
        ...
        if (tab.url == "chrome://newtab/") $("body").append($('<p>').html(chrome.i18n.getMessage('popup_status_blocked_newtab2', ["<a href='https://github.com/jk779/Clean-Gestures' target='_blank'>" + chrome.i18n.getMessage('popup_status_blocked_newtab2_link') + "</a>"])));
        if (tab.url.match(/^file:\/\//)) $("body").append($('<p>').html(chrome.i18n.getMessage('popup_status_blocked_file2', '<a href="chrome://extensions/" target="_blank">' + chrome.i18n.getMessage('popup_status_blocked_file2_link') + '</a>')));
        $("body").append($('<div class=sgbutton>').text(chrome.i18n.getMessage('popup_button_options')).css({
          "float": "right"
        }).click(function () {
          chrome.tabs.create({
            "url": "options.html"
        ...
        $("body").append("<div id=statuslight class=green>").append($("<h1>").text(chrome.i18n.getMessage('popup_status_working_short')))
          ...
          .append($('<div class=sgbutton>').text(chrome.i18n.getMessage('popup_button_hidepageaction')).css({
          "float": "left"
        }).click(function () {
