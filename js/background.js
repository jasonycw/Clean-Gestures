...
var nav = navigator.platform.match(/Win/) ? "win" : (navigator.platform.match(/Mac/) ? "mac" : (navigator.platform.match(/Linux/) ? "linux" : "unknown"));
...
var categories = {
  'cat_page_navigation': {
    actions: ['page-back', 'page-forward', 'page-back-close', 'reload-tab', 'reload-tab-full', 'reload-all-tabs', 'stop', 'page-next', 'page-prev']
  },
  'cat_tab_management': {
  ...
  'cat_utilities': {
    actions: ['goto-top', 'goto-bottom', 'page-up', 'page-down', 'print', 'parent-dir', 'view-source', 'show-cookies', 'search-sel', 'zoom-in', 'zoom-out', 'zoom-zero', 'open-image', 'hide-image', 'zoom-img-in', 'zoom-img-out', 'zoom-img-zero', 'translate', 'copy', 'toggle-bookmark', 'bookmark', 'unbookmark']
  },
  'cat_other': {
    actions: ['options', 'status', 'open-history', 'open-downloads', 'open-extensions', 'open-bookmarks']
  },
  'cat_hacks': {
    actions: ['undo-close-hack', 'clone-tab-hack', 'prev-window', 'next-window', 'maximize-x', 'minimize-x']
  },
  'cat_custom': {
    customActions: true
  },
...
var actions = {
  ...
  'navigate-tab': function (id, call) {
    chrome.tabs.update(contents[id].detail.tabId, {
      url: settings.newTabUrl != "homepage" ? settings.newTabUrl : undefined
    }, call);
  },
    ...
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      if (tab.pinned) call();
        ...
        chrome.windows.getAll({
          ...
          if (wins.length == 1 && wins[0].tabs.length == 1) chrome.tabs.update(contents[id].detail.tabId, {
            url: settings.newTabUrl != "homepage" ? settings.newTabUrl : undefined
          }, call);
  ...
  'undo-close': function (id, call) {
    if (pluginport) {
      pluginport.postMessage(JSON.stringify({
        sendkey: {
          keys: ["t"],
          mods: [navMac ? "alt" : "ctrl", "shift"]
        }
      }));
      call();
    } else {
  ...
  'undo-close-hack': function (id, call) {
    ...
    if (t) chrome.windows.get(t.winId, function (win) {
      if (t.history.length > 1) {
        ...
        var url = "http://localhost/?index=0#:--:" + r + ":--:" + escape(JSON.stringify(t.titles)) + ":--:" + escape(JSON.stringify(t.history));
      } else {
        var url = t.history[0];
      }
      if (win) chrome.tabs.create({
  ...
  'reload-all-tabs': function (id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      chrome.tabs.getAllInWindow(tab.windowId, function (tabs) {
        for (i = 0; i < tabs.length; i++) chrome.tabs.update(tabs[i].id, {
          url: tabs[i].url
        });
        call();
      });
  ...
  'merge-tabs': function (id, call) {
    chrome.tabs.getAllInWindow(null, function (tabs) {
      ...
      if (winId) {
        for (i = 0; i < tabs.length; i++) chrome.tabs.move(tabs[i].id, {
          windowId: winId,
          index: 1000000
        });
        chrome.tabs.update(contents[id].detail.tabId, {
  ...
  'status': function (id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      chrome.tabs.create({
        url: chrome.extension.getURL("status.html"),
        windowId: tab.windowId
      }, call);
    });
  },
  'page-back-close': function (id, call) {
    runJS(id, "history.back();" + (activeTabs[contents[id].detail.tabId].history.length == 1 ? " setTimeout(function(){ port.postMessage(JSON.stringify({closetab:true})); },400);" : ""));
    call();
  },
  'goto-top': function (id, call, a) {
    runJS(id, (a.startPoint ? "for(var t = document.elementFromPoint(" + a.startPoint.x + "," + a.startPoint.y + "); t!=document.documentElement && t!=document.body && (t.scrollHeight<=t.clientHeight || t.scrollTop==0 || ['auto','scroll'].indexOf(document.defaultView.getComputedStyle(t)['overflow-y'])==-1); t = t.parentNode); if(t == document.documentElement) t = document.body; t.scrollTop = 0" : "document.body.scrollTop = 0"));
    call();
  },
  ...
  'clone-tab-hack': function (id, call) {
    ...
    if (t) chrome.windows.get(t.winId, function (win) {
      if (tabhistory.length > 1) {
        ...
        var url = "http://localhost/?index=0#:--:" + r + ":--:" + escape(JSON.stringify(tabtitles)) + ":--:" + escape(JSON.stringify(tabhistory));
      } else {
  ...
  'maximize-x': function (id, call) {
    chrome.windows.getLastFocused(function (win) {
      if (!maximizeXWindow[win.id]) {
        maximizeXWindow[win.id] = {
          dim: win
        };
        chrome.windows.update(win.id, {
          left: (screen.availWidth > screen.availHeight && win.left >= screen.availWidth / 2 ? screen.availWidth / 2 : 0),
          top: 0,
  ...
  'zoom-in': function (id, call) {
    if (pluginport) {
      pluginport.postMessage(JSON.stringify({
        sendkey: {
          keys: ["+"],
          mods: [navMac ? "alt" : "ctrl"]
        }
      }));
      call();
    } else {
      runJS(id, "var zoom = document.body.style.zoom ? document.body.style.zoom*1.1 : 1.1; document.body.style.zoom = zoom; canvas.style.zoom = 1/zoom;");
      call();
    }
  },
  'zoom-out': function (id, call) {
    if (pluginport) {
      ...
      runJS(id, "var zoom = document.body.style.zoom ? document.body.style.zoom*(1/1.1) : 1/1.1; document.body.style.zoom = zoom; canvas.style.zoom = 1/zoom;");
      call();
    }
  },
  ...
  'tab-to-left': function (id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      chrome.tabs.move(tab.id, {
        index: tab.index > 0 ? tab.index - 1 : 0
      });
    });
  },
  ...
  'parent-dir': function (id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      ...
      if (url) chrome.tabs.update(tab.id, {
        url: url
      }, call);
      else call();
    });
  ...
  'show-cookies': function (id, call) {
    var l = 100;
    var m = 5;
    runJS(id, "window.alert('Cookies stored by this host or domain:\\n'+('\\n'+document.cookie).replace(/; /g,';\\n').replace(/\\n(.{" + (l * 2 - 8) + "})([^\\n]{" + (m) + "})/gm,\"\\n$1\\n        $2\").replace(/\\n(.{" + (l) + "})([^\\n]{" + (m) + "})/gm,\"\\n$1\\n        $2\"));", true);
    call();
  },
  'search-sel': function (id, call, a) {
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      chrome.tabs.create({
        url: "http://localhost/search?q=" + a.selection,
        windowId: tab.windowId,
        index: tab.index + 1
      }, call);
    });
  ...
  'toggle-pin': function (id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      if (chromeVersion >= 9) chrome.tabs.update(tab.id, {
        pinned: !tab.pinned
      }, call);
    });
  },
  ...
  'toggle-bookmark': function (id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function (tab) {
      chrome.bookmarks.search(tab.url, function (bmks) {
        if (bmks.length <= 0) chrome.bookmarks.create({
          parentId: "2",
          title: tab.title,
          url: tab.url
        }, call);
...
var focusWindow = function (winId, call) {
  if (focusedWindows.length <= 1) call();
  else chrome.tabs.getSelected(winId, function (tab) {
...
chrome.extension.onRequest.addListener(function (request, sender, respond) {
  if (request.getstates) {
    ...
    switch (request.requestType) {
      ...
      default:
        respond({
          status: "unrecognized request type"
        });
    }
...
chrome.extension.onConnectExternal.addListener(function (port) {
  if (port.name == "sgplugin" && (port.sender.id == "bkojnmffeemeacfnhgiighecdfojgpdm" || port.sender.id == "apagmdofhjomjncpiebpaaonngppcpcl")) {
    ...
    pluginport.onDisconnect.addListener(function () {
      pluginport = null;
      for (id in contents) contents[id].postMessage(JSON.stringify({
        sgplugin: false
      }));
    });
    for (id in contents) contents[id].postMessage(JSON.stringify({
...
var contentMessage = function (id, mess) {
  ...
  if (mess.gesture && gestures[mess.gesture]) {
    ...
    chainGesture = null;
    if (mess.gesture[0] == "r") chainGesture = {
      rocker: true,
      timeout: setTimeout(function () {
        chainGesture = null
      }, 2000)
    };
    if (mess.gesture[0] == "w") chainGesture = {
      wheel: true,
      timeout: setTimeout(function () {
        chainGesture = null
      }, 2000)
    ...
    var call = !chainGesture ? null : function () {
        chrome.tabs.getSelected(null, function (tab) {
          ...
          for (id in contents)
          if (tab.id == contents[id].detail.tabId) contents[id].postMessage(JSON.stringify({
            chain: chainGesture
          }));
        });
      }
    try {
      if (actions[gestures[mess.gesture]]) actions[gestures[mess.gesture]].call(null, id, call, mess);
      else if (externalActions[gestures[mess.gesture].substr(0, 32)]) chrome.extension.sendRequest(gestures[mess.gesture].substr(0, 32), {
    ...
    if (!log.action[gestures[mess.gesture]][mess.gesture]) log.action[gestures[mess.gesture]][mess.gesture] = {
      count: 0
    };
    log.action[gestures[mess.gesture]][mess.gesture].count += 1;
    if (!log.line) log.line = {
      distance: 0,
      segments: 0
    };
    if (mess.line) {
  ...
  if (mess.sgplugin && pluginport) {
    if (mess.sgplugin.rightclick) pluginport.postMessage(JSON.stringify({
      sendbutton: 3
    }));
  }
}
...
var selectionChanged = function (tabId) {
  ...
  for (id in contents)
  if (selectedTabId == contents[id].detail.tabId) contents[id].postMessage(JSON.stringify({
    windowBlurred: true
  }));
...
var updateActiveTab = function (tabId, change) {
  chrome.tabs.get(tabId, function (tab) {
    ...
    if (tab.url.substr(0, 29) == "http://localhost/?index=") {
    ...
    if (!activeTabs[tabId]) activeTabs[tabId] = {
      history: [],
      titles: []
    };
    ...
    if (pluginport && tab.selected) {
      if (tab.status == "complete" && tab.url.match(/^chrome-extension:.*Smooth_Gestures_Settings.html$/)) pluginport.postMessage(JSON.stringify({
        sendkey: {
          keys: ["s"],
          mods: [navMac ? "alt" : "ctrl"]
        }
...
var saveOptions = function (profile, call) {
  ...
  //update all tabs
  for (id in contents)
  contents[id].postMessage(JSON.stringify({
    settings: settings,
    validGestures: validGestures
  }));
  ...
  setTimeout(function () {
    getTabStates(function (states) {
      ...
      chrome.bookmarks.getTree(function (tree) {
        ...
        for (var i = 0; i < other.children.length; i++) if ((other.children[i].title == "GestureSync" || other.children[i].title == "Smooth Gestures Sync")) //&& other.children[i].children)
...
var connectExistingTabs = function () {
  chrome.windows.getAll({
    ...
    for (x in wins) {
      if (focusedWindows.indexOf(wins[x].id) < 0) focusedWindows.push(wins[x].id);
