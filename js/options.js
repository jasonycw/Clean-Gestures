...
$("body").append($('<div>').attr('id', 'contents-contain').append($('<div>').attr('id', 'contents')
  ...
  .append($('<li id="supportmess">Find a bug? <a id="supportlink" href="https://github.com/jk779/Clean-Gestures/issues">Report it</a></li>')))))
  .append($('<h1 id="optionstitle"><img src="im/icon48.png"/> ' + getText("name") + ': ' + getText("options_title") + '</h1>'))
  .append($('<div>').attr('id', 'addgesture')
  .append($("<div>").attr("id", "addgesturebutton").html("<span>+</span> " + chrome.i18n.getMessage("options_button_startaddgesture")).attr("tabindex", 0).click(function () {
  addGesture(null);
})))
if (bg.settings.sendStats == undefined) $("#optionstitle").after($('<div>').attr('id', 'note_sendStats').attr('class', 'note green')
...
if (bg.settings.hidePageAction == undefined) $("#optionstitle").after($('<div>').attr('id', 'note_hidePageAction').attr('class', 'note green')
  .append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
  ...
  .append($("<div>").attr("class", "clearall")))
if (!bg.settings.hideNoteTranslate && ['en', 'en-US', 'en-GB'].indexOf(navigator.language) == -1) $("#optionstitle").after($('<div>').attr('id', 'note_translate').attr('class', 'note green')
  .append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
  ...
  .append($("<div>").attr("class", "clearall")))
if (!bg.settings.hideNotePrint) $("#optionstitle").after($('<div>').attr('id', 'note_print').attr('class', 'note green')
  .append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
  $("#note_print").remove();
  ...
  .append($("<div>").attr("class", "clearall")))
if (!bg.settings.hideNoteWelcome) $("#optionstitle").after($('<div>').attr('id', 'note_welcome').attr('class', 'note clear')
  .append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
  $("#note_welcome").remove();
...
if (!bg.settings.hideNoteRemindRate /* && new Date() - new Date(bg.getValue('date_first')) > 30*24*60*60*1000*/ ) {
  $("#optionstitle").after($('<div>').attr('id', 'note_remindrate').attr('class', 'note green')
    ...
    .append($('<p>').html(getText("options_note_remindrate", "<a href='https://github.com/jk779/Clean-Gestures'>" + getText("options_note_remindrate_link") + "</a>")))
    .append($("<div>").attr("class", "clearall")))
}
...
var generateGRow = function (action) {
  return $("<div>").attr("class", 'actionrow')
    ...
    .append($("<div>").attr("class", "gesture-add").attr("tabindex", 0).text("+").click(function () {
    addGesture(action)
  }))
...
var generateCustomRow = function (id) {
  return $("<div>").attr("class", 'actionrow')
    ...
    .append($("<span>").attr("class", "editcustom").attr("tabindex", 0).text("edit").click(function () {
    editCustom(id)
  }))
...
var editCustom = function (id) {
  ...
  var edit = $("<div>").attr("class", "actionrow").attr("id", "edit-" + id)
    .append($("<div>").attr("class", "savecustom").attr("tabindex", 0).text("save").click(function () {
    saveCustom(id);
  }))
...
var addCustom = function () {
  ...
  a.code = "location.href = \"https://www.google.com\";";
  a.env = "page";
  a.share = true;
  a.context = "";
  bg.contexts[id] = a.context;
...
var rowReset = function () {
  return $('<div>').attr('class', "settingrow")
    ...
    .append($("<input>").attr("type", "button").val(getText("setting_button_reset")).css({
    'min-width': '5em',
    'font-weight': 'bold'
  }).click(doReset)))
...
var rowImport = function () {
  return $('<div>').attr('class', "settingrow")
    ...
    .append($("<div>").attr("class", "settingcontrol")
    .append($("<input>").attr("type", "button").val(getText("setting_button_export")).css({
    'min-width': '5em',
    'font-weight': 'bold'
  }).click(function (t) {
    chrome.tabs.create({
...
var rowTrailColor = function () {
  return $('<div>').attr("class", "settingrow")
    .append($("<div>").attr("class", "settingcontrol")
    .append($("<div>").attr("id", "trailColor").css({
    "float": "right",
    "width": "15px",
    "height": "140px",
    ...
    .append($("<div>").attr("id", "trailColorG").append($("<input>").attr({
    type: "range",
    min: 0,
    max: 255,
    value: bg.settings.trailColor.g
  }).change(updateTrailColor)))
    .append($("<div>").attr("id", "trailColorB").append($("<input>").attr({
    type: "range",
    min: 0,
    max: 255,
    value: bg.settings.trailColor.b
  }).change(updateTrailColor)))
    .append($("<div>").attr("id", "trailColorA").append($("<input>").attr({
    type: "range",
    min: 0,
    max: 1,
    step: .01,
...
var rowNewTabUrl = function () {
  return $('<div>').attr("class", "settingrow")
    ...
    .append($("<input>").attr("id", "newTabUrlCustom").attr("type", "text").css({
    "width": "160px",
    "border": "1px solid gray",
    "margin": "0 .8em"
  }))
...
var updateNewTabUrl = function () {
  ...
  if (!bg.settings.newTabUrl.match(/:/) && bg.settings.newTabUrl.match(/\./)) {
    bg.settings.newTabUrl = "https://www.google.com/";
  }
...
var rowTrailWidth = function () {
  return $('<div>').attr("class", "settingrow")
    .append($("<div>").attr("class", "settingcontrol")
    .append($("<div>").attr("id", "trailWidthDraw").css({
    "float": "right",
    "width": "100px",
    "height": "140px"
  }))
...
var displayGesture = function (gesture) {
  $('#action-' + bg.gestures[gesture])
    ...
    .append($('<div>').attr("class", 'gesture-remove').attr("tabindex", 0).html('&times;').click(function () {
...
var addGesture = function (action) {
  ...
  var receiveGesture = function (gesture) {
    ...
    $("<div>").attr("id", "nowwhat").css({
      ...
      .append(action ? $("<input>").attr("type", "button").val(bg.gestures[gesture] ? getText("options_button_overwrite") : getText("options_button_addgesture"))
      .css({
      "font-size": "1.1em",
      "float": "right",
      "min-width": "10em",
