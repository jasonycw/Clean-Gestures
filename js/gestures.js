...
var SmoothGestures = function () {
  ...
  var mouseDownCapture = function (t) {
    ...
    //block scrollbars
    if ((t.target && t.target.nodeName == "HTML") && ((document.height > window.innerHeight && t.clientX > window.innerWidth - 17) || (document.width > window.innerWidth && t.clientY > window.innerHeight - 17))) {
      endGesture();
      return;
    }
    if (syncButtons) port.postMessage(JSON.stringify({
      syncButton: {
        id: t.button,
        down: true
      }
    ...
    t.button == settings.holdButton, (buttonDown[0] ? 1 : 0) + (buttonDown[1] ? 1 : 0) + (buttonDown[2] ? 1 : 0) == 1 && (_this.callback || (validGestures['r'] && ((buttonDown[0] && validGestures['r']['L']) || (buttonDown[1] && validGestures['r']['M']) || (buttonDown[2] && validGestures['r']['R'])))),
  ...
  var mouseUpCapture = function (t) {
    ...
    if (t.button == 2 && !forceContext && !blockContext && !buttonDown[0] && !buttonDown[1] && navigator.platform.indexOf("Win") == -1) {
      ...
      if (sgplugin) {
        ...
        if (!settings.blockDoubleclickAlert) port.postMessage(JSON.stringify({
          alertDoubleclick: {
  ...
  var keyDownCapture = function (t) {
    ...
    if (t.keyCode == 16 || t.keyCode == 17 || t.keyCode == 18 || t.keyCode == 0 || t.keyCode == 92 || t.keyCode == 93) {
      ...
      (t.keyCode == 17 ? 0 : //ctrl
      (t.keyCode == 18 ? 1 : //alt
      (t.keyCode == 0 ? null : (t.keyCode == 92 ? null : (t.keyCode == 93 ? null : null))))));
      if (i != null) {
      ...
      keyMod = mod;
    } else if (_this.callback || ((mod != "0000" || focus == null || (focus.nodeName != "INPUT" && focus.nodeName != "TEXTAREA")) && validGestures["k"] && validGestures["k"][mod] && validGestures["k"][mod].indexOf(t.keyIdentifier + ":" + t.keyCode) >= 0)) {
  ...
  var startGesture = function (point, target, line, rocker, wheel, time) {
    ...
    gesture.wheel = wheel;
    if (document.documentElement.offsetHeight < document.documentElement.scrollHeight && (gesture.line || gesture.wheel) && !htmlclear.parentNode) document.body.appendChild(htmlclear);
  };
  var moveGesture = function (t, diagonal) {
    if (!gesture.startPoint) gesture.startPoint = {
      x: t.clientX,
      y: t.clientY
    };
    ...
    if (gesture.line) {
      ...
      if (!settings.trailBlock && canvas.getContext) {
        ...
        ctx.stroke();
        refreshLineAsync();
        if (!canvas.parentNode && (Math.abs(diffx) > 10 || Math.abs(diffy) > 10)) document.body.appendChild(canvas);
      }
      ...
      if (ndir == ldir) {
        ...
        if (gesture.line.possibleDirs || _this.callback) {
          ...
          if (gesture.line.dirPoints.length > 1) gesture.line.dirPoints.push(next);
  ...
  var sendGesture = function (code) {
    if (code) {
      if (_this.callback) {
        ...
        if (gesture.line && code[0] != "w" && code[0] != "r") message.line = {
        ...
        if (settings.selectToLink && gesture.selection) {
          ...
          for (var i = 1; i < parts.length; i++) {
            ...
            link = link.split(/[\s"']/)[0];
            if (link.match(/\/\/.+\..+/)) message.links.push({
              src: link
            });
          }
        }
        for (var i = 0; i < gesture.targets.length; i++) {
          ...
          if (link) message.links.push({
            src: link,
            gestureid: gestureid
          });
          if (gesture.targets[i].nodeName == "IMG") message.images.push({
  ...
  _this.drawGesture = function (gesture, width, height, lineWidth) {
    ...
    if (!mess) return c;
      ...
      .append($("<div>").css({
      "font-size": (.8 * Math.sqrt(width / 100)) + "em",
      "color": "#888",
      "text-align": "right",
      "margin-right": ".3em"
    }).text(mess))
  ...
  var drawRocker = function (gesture, width) {
    ...
    return $("<div>").css({
      ...
      .append($("<div>").text(chrome.i18n.getMessage('gesture_rocker_descrip', [chrome.i18n.getMessage('options_mousebutton_' + first), chrome.i18n.getMessage('options_mousebutton_' + second)]))
      .css({
      "font-size": (.8 * Math.sqrt(width / 100)) + "em",
      "color": "#666",
      "text-align": "center"
